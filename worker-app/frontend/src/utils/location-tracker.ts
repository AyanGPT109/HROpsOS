...
export class ForegroundLocationTracker implements LocationTracker {
  ...
  async start(config: LocationTrackingConfig): Promise<void> {
    ...
    try {
      ...
      if (permission.location !== 'granted') {
        config.onEvent('error', undefined, new Error('Location permission denied.'))
        return
      }
    } catch (e) {
      config.onEvent('error', undefined, e instanceof Error ? e : new Error('Permission error'))
      return
    }
    ...
    try {
      this.watchId = await Geolocation.watchPosition(
        ...
        (pos, err) => {
          if (err) {
            config.onEvent('error', undefined, new Error(err.message || 'Watch error'))
            return
          }
          if (pos) {
      ...
      config.onEvent('error', undefined, e instanceof Error ? e : new Error('Failed to start watch'))
    }
    ...
    this.intervalId = setInterval(async () => {
      try {
        ...
        config.onEvent('error', undefined, err instanceof Error ? err : new Error('Interval error'))
      }
    }, config.intervalMs)
  ...
  private handlePosition(pos: { coords: { latitude: number; longitude: number; accuracy: number }; timestamp: number }, heartbeat = false) {
