...
async function getWorkerContext() {
  ...
  if (!worker) throw new Error('No active worker record is linked to this account')
  return worker
}
...
export const attendanceService = {
  async getToday(workerId?: string): Promise<Attendance | null> {
    ...
    if (!workerId) {
      return employeeGet<Attendance | null>('/api/employee/attendance/today')
    }
  ...
  async checkIn(payload: CheckInPayload): Promise<Attendance> {
    ...
    const data = await employeePost<Attendance>('/api/employee/attendance/upsert', {
      row,
      onConflict: 'worker_id,date',
    })

    await employeePost('/api/employee/attendance/log', {
      row: {
        worker_id: worker.id,
        company_id: worker.company_id,
        plant_id: worker.plant_id,
  ...
  async checkOut(payload: CheckInPayload): Promise<Attendance> {
    ...
    const data = await employeePost<Attendance>('/api/employee/attendance/update', {
      id: existing.id,
      row: {
        status,
        check_out_at: now,
